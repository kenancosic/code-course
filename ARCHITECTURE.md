...
```json
{
  ...
  "connections": [
    { "from_node_id": "html-basics", "to_node_id": "css-basics", "type": "required" }
  ]
}
```
...
```json
{
  ...
  "achievements": [
    { "id": "first_blood", "title": "First Blood", "description": "Solved a syntax error without Google.", "unlocked": true, "unlocked_at": "2026-01-15T10:30:00Z" }
  ]
}
```
...
erDiagram
    ...
    ROADMAP_NODES ||--o{ COURSES : generates
...
Individual topics within a roadmap path.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `TEXT` | PK | Slug identifier, e.g. `html-basics` |
| `path_id` | `TEXT` | FK → `roadmap_paths.id`, NOT NULL | Parent path |
| `title` | `TEXT` | NOT NULL | Display title, e.g. `HTML Glyphs` |
| `description` | `TEXT` | NOT NULL | Short description of the topic |
| `position_x` | `REAL` | NOT NULL | X position on roadmap canvas (0-100) |
| `position_y` | `REAL` | NOT NULL | Y position on roadmap canvas (0-100) |
| `tier` | `INTEGER` | NOT NULL DEFAULT 1 | Difficulty tier (1=beginner ... 5=expert) |
| `topic_keywords` | `TEXT` | | Comma-separated keywords for LLM context |
| `created_at` | `TIMESTAMP` | NOT NULL DEFAULT NOW | — |
...
| `roadmap_node_id` | `TEXT` | FK → `roadmap_nodes.id`, NULLABLE | Source node (NULL if from PDF) |
| `pdf_id` | `TEXT` | FK → `uploaded_pdfs.id`, NULLABLE | Source PDF (NULL if from topic) |
| `model_used` | `TEXT` | NOT NULL | LLM model that generated the content |
...
|--------|------|-------------|-------------|
| `id` | `TEXT` | PK | UUID |
| `course_id` | `TEXT` | FK → `courses.id` ON DELETE CASCADE, NOT NULL | Parent course |
| `title` | `TEXT` | NOT NULL | Lesson title |
| `content_markdown` | `TEXT` | NOT NULL | Full lesson content in Markdown |
| `code_examples` | `TEXT` | | JSON array of code snippets |
| `exercises` | `TEXT` | | JSON array of exercise definitions |
