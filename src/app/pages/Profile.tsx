import { Shield, Target, Award, Swords, Compass, Circle, CheckCircle, BookOpen, Trophy } from "lucide-react";
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mt-6">
            <div className="flex flex-col text-foreground bg-card/60 p-3 rounded-lg border border-border/50">
              <span className="text-xs uppercase tracking-wider text-muted-foreground/70 font-semibold">Current Path</span>
              <span className="font-bold text-primary flex items-center gap-2">
                <Compass className="w-4 h-4" /> {profile?.current_path?.title || "Explorer"}
              </span>
            </div>
            <div className="flex flex-col text-foreground bg-card/60 p-3 rounded-lg border border-border/50">
              <span className="text-xs uppercase tracking-wider text-muted-foreground/70 font-semibold">Total XP</span>
              <span className="font-bold text-chart-1 flex items-center gap-2">
                <Award className="w-4 h-4" /> {profile?.total_xp || 0}
              </span>
            </div>
            <div className="flex flex-col text-foreground bg-card/60 p-3 rounded-lg border border-border/50">
              <span className="text-xs uppercase tracking-wider text-muted-foreground/70 font-semibold">Quests Conquered</span>
          ...
          <CardContent className="space-y-6">
            {profile?.skills && profile.skills.length > 0 ? (
              profile.skills.map((skill, index) => {
                // Backend provides level and xp directly. Let's calculate progress to next level (assuming 1000 xp per level for visual representation)
                const skillLevel = skill.level || 1;
                // Just for visual flair, using a pseudo-progress based on xp
                const progress = ((skill.xp || 0) % 1000) / 10;
                return (
                  <SkillBar
                    key={skill.name}
                    name={skill.name}
              ...
              <p className="text-muted-foreground/70 text-center py-4">No skills unlocked yet. Complete lessons to gain skills!</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-amber-500/20 bg-card/40">
          ...
          <CardContent>
            {achievementsLoading ? (
              ...
              <p className="text-muted-foreground/70 text-center py-4">No achievements unlocked yet. Keep exploring to earn relics!</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-emerald-500/20 bg-card/40">
        ...
        <CardContent>
          {profile?.recent_activity && profile.recent_activity.length > 0 ? (
            <div className="space-y-3">
              {profile.recent_activity.slice(0, 5).map((activity) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-medium text-foreground capitalize">{activity.type.replace('_', ' ')}</h4>
            ...
            <p className="text-muted-foreground/70 text-center py-4">No recent activity. Start your adventure today!</p>
          )}
        </CardContent>
      </Card>
    </div>
    ...
    <div
      className={`flex flex-col items-center text-center p-4 rounded-xl border transition-all ${
        active
          ...
          : "bg-card/30 border-border opacity-50 grayscale"
      }`}
    >
